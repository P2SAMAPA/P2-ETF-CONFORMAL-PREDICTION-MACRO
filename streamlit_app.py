...
st.set_page_config(page_title="Conformal Prediction with Macro‑Adaptive Non‑conformity", layout="wide")
...
st.markdown('<h1 style="text-align: center;">📊 Conformal Prediction with Macro‑Adaptive Non‑conformity</h1>', unsafe_allow_html=True)
st.markdown('<p style="text-align: center;">Distribution‑free prediction intervals | Macro‑weighted non‑conformity | Confidence score</p>', unsafe_allow_html=True)

st.sidebar.markdown("## 🧮 Conformal Prediction")
...
st.sidebar.markdown(f"**Coverage:** {int(config.CONFIDENCE*100)}% | **Trees:** {config.N_ESTIMATORS}")
...
def find_latest_json(files):
    json_files = [f for f in files if f.endswith('.json') and 'conformal_prediction_' in f]
    if not json_files:
        return None
...
def display_universe(universe_name, uni_data, window_data, window_label):
    ...
    for idx, etf in enumerate(top3):
        with cols[idx]:
            ...
            <div class="hero-card">
                <h3>{etf['ticker']}</h3>
                <p>Confidence score: {etf['conf_score_norm']:.3f}</p>
                <p style="font-size:0.9rem;">raw: {etf['raw_score']:.4f}</p>
            </div>
            """, unsafe_allow_html=True)
    with st.expander(f"Full ranking for {universe_name}"):
        df_full = pd.DataFrame(list(norm_scores.items()), columns=["Ticker", "Normalized Confidence"])
        df_full["Raw Score"] = df_full["Ticker"].apply(lambda t: raw_scores[t])
        df_full = df_full.sort_values("Normalized Confidence", ascending=False)
        st.dataframe(df_full, use_container_width=True)

tab1, tab2 = st.tabs(["📊 Best Window (Auto)", "🔍 Choose Window (Manual)"])

with tab1:
    st.header("📊 Top ETFs by Conformal Prediction Confidence (Auto Best Window)")
    with st.expander("📖 Interpretation", expanded=False):
        st.markdown("""
        - **Conformal prediction** provides distribution‑free prediction intervals with guaranteed coverage.
        - A random forest regressor predicts next‑day return from macro variables.
        - The **non‑conformity score** (absolute residual) is **weighted by a composite macro factor** (from all macro variables: VIX, DXY, yields).
        - The score = 1 / (1 + interval width) – higher means a **narrower, more confident prediction**.
        - High score → ETF’s next‑day return is highly predictable given macro (potential alpha).
        - Low score → uncertain prediction.
        """)
...
with tab2:
    ...
    st.markdown("Choose a rolling window to inspect the confidence scores.")
    for universe_name, uni_data in data["universes"].items():
        if not uni_data or not uni_data.get("all_windows"):
            st.warning(f"No window data for {universe_name}")
            continue
...
st.sidebar.markdown("---")
st.sidebar.caption("Conformal Prediction | Macro‑adaptive non‑conformity for prediction intervals")
